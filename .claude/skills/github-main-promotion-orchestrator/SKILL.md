...
- Do not run a separate pre-PR review loop before the first `stage -> main` PR. After main-promotion preflight is `go`, convert directly to the real GitHub PR for the pertinent repo/lane. For `main`, the review surface is the PR itself: checks plus GitHub review comments, including Copilot/Codex comments.
- Once authoritative `stage` evidence is green and main preflight is `go`, promotion must convert directly to PR/wait/merge flow. Do not reopen local review loops, derived remediation branches, or speculative reruns unless source heads changed, the applicable CI-equivalent invalidation logic requires renewed evidence, or a newly frozen finding is classified as `release-blocker`.
- Load `ci-equivalent-governance` before deciding which local matrix or stage-parity contract satisfies the lane's `CI-Equivalent` prerequisite.
- If accepted remediation changes any stage-facing test row, wrapper, lifecycle step, or readonly/mutation coverage row, load `ci-equivalent-test-surface-admission` before resuming the lane.
...
- Run the deterministic main-promotion preflight before opening the first PR in the lane:
  ...
  - The preflight validates: stage health per repo, promotable diff beyond main, green push runs, and Docker submodule alignment. In app-first scenarios, Docker-pinned app stage SHAs are the promotion authority; newer unrelated app `dev` commits do not by themselves block opening the pertinent app `stage -> main` PRs.
- After main-promotion preflight returns `Overall outcome: go`, open the real `stage -> main` PR for the pertinent repo/lane. Do not require `copilot-pr-review` or any separate no-context review preflight before PR creation. Use the GitHub PR review surface itself as the review gate: inspect Copilot/Codex/other PR comments, compare them against the governing TODO, and run `review-finding-classification` after findings are collected. Loop only when a PR finding is confirmed as `release-blocker` after TODO-decision review.
- If a remediation wave changes a stage-facing test/harness surface during that loop, complete `ci-equivalent-test-surface-admission` before declaring either branch CI-equivalent clean.
...
- **PR review surface:** Do not run `copilot-pr-review` as a preflight requirement for `stage -> main`. Use the actual GitHub PR review surface, including Copilot comments, and route collected findings through `review-finding-classification` before merge decisions.
...
3. Classify the finding in two stages:
   ...
   Reviewers keep their normal detection behavior; the second-stage routing happens only after findings are collected and must run through `review-finding-classification`.
