---
name: wf-docker-session-lifecycle-method
description: "Workflow: MUST use whenever the scope matches this purpose: Ensure every Delphi session respects instruction-loading rules, especially during Self Improvement Sessions, and that no architectural work proceeds with stale directives. This method formalises how sessions start, run, and end."
---
...
1. **Session Start**
   ...
   - Record subagent/delegation authorization separately from worktree/auxiliary-checkout authorization. Default Git topology to the principal checkout with one writer at a time unless the human explicitly names worktrees or auxiliary checkouts.
