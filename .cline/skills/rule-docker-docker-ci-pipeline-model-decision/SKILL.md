...
## Notes
Align pipeline steps with existing workflow commands (analyze/tests) from stack-specific rules, load `ci-equivalent-governance` whenever a CI edit changes stage-facing suite/job ownership or a broad local stage gate such as `stage-full`, and load `ci-equivalent-test-surface-admission` whenever the edit changes how tests/wrappers/lifecycle rows enter that stage-facing surface.
