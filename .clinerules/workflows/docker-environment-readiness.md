...
<!-- Generated from `workflows/docker/environment-readiness-method.md` by `tools/sync_clinerules_mirrors.py`. Do not edit directly. -->
...
2. **Run canonical readiness scripts (preferred)**
   ...
   - Treat ignored/local env files as project-owned operational authority, not disposable noise. If a project relies on files such as root `.env`, `.env.local.*`, or submodule-local `.env`, do not delete or recreate them from `*.example` during cleanup, reconcile, or "environment normalization" unless the user explicitly authorizes replacement or a validated local backup/runtime source has already been identified.
   - If a readiness failure suggests one of those ignored env files is missing or stale, stop mutation first. Recover the last known-good values from project-owned evidence (current running containers/process args, validated local backups, runtime-surface snapshots, README-declared local overlays, or explicit user confirmation), restore the ignored file, rerun readiness, and only then restart services or continue CI/promotion work.
