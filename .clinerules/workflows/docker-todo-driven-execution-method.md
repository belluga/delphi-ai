...
17. **Build Execution Plan (mandatory before `APROVADO`)**
   ...
   - Build item-level coverage from the TODO contract:
     ...
     - any user-visible, interactive, or user-flow-impacting item must map to an integration/device test or navigation/browser test that exercises that exact item;
     - user-flow impact is not limited to criteria worded as visual UI. CRUD/mutation is a strong signal, but field refactors, DTO/domain/payload shape changes, backend validation, request/response projections, query/filter semantics, settings/capabilities, read models, and persisted state changes must be assessed case by case;
     - if a changed surface can affect an admin/public screen, save/readback flow, list/detail rendering, filter/search result, or persisted user-visible state, default to requiring runtime flow evidence unless the TODO records why no user-observable behavior can change;
     ...
     - classify each flow-impacting item as `android-only`, `web-only`, `shared-android-web`, or `divergent-android-web`; when behavior is shared, either ADB integration or Playwright navigation may close the final runtime lane, but when behavior differs materially between Android and Web, both lanes are required;
     - any browser/web-visible item must map to a source-owned Playwright spec and runner when the downstream repository exposes a Playwright web suite; for Flutter web, use `tools/flutter/web_app_tests/**` plus the project-owned navigation runner only after publishing the current checkout with the project-defined build/publish command and output target from `foundation_documentation` or dependency-readiness notes, then confirming the real browser-facing domain serves that refreshed bundle;
...
22. **Completion Evidence Matrix Gate (mandatory before delivery claim)**
   ...
   - For every user-visible, interactive, or user-flow-impacting criterion, evidence must name the exact integration/device test or navigation/browser test that exercises the item. In Flutter scope, integration is ADB/device execution and navigation/browser is Playwright against the final browser-facing domain. Code inspection, analyzer output, unit tests, widget tests, and screenshots are valid supporting evidence, but final orchestrator acceptance needs the runtime lane unless an approved structure-only waiver exists.
   - User-flow-impacting criteria include more than visual UI and CRUD/mutation. Field refactors, DTO/domain/payload shape changes, backend validation, request/response projections, query/filter semantics, settings/capabilities, read models, and persisted state changes must be assessed case by case. If they can affect a screen, save/readback flow, list/detail surface, search/filter result, or persisted user-visible state, runtime flow evidence is required unless an explicit non-applicability rationale is recorded.
   ...
   - For every browser/web-visible criterion, evidence must name the exact Playwright spec and runner command when the repository exposes a Playwright suite. Flutter web evidence must use the source-owned specs under `tools/flutter/web_app_tests/**`, executed through the project-owned navigation runner after the project-defined build/publish command publishes the current checkout and the real browser-facing domain is confirmed to serve that refreshed bundle.
   - For criteria involving user-flow CRUD/mutation, evidence must show the integration/device or navigation/browser test performed the local mutation path on the approved non-main validation target. Read-only navigation, local-only UI filtering, and backend-only assertions do not satisfy mutation criteria when a user-facing save/readback flow can change.
...
27. **Validate**
   ...
   - Cross-check every user-visible, interactive, or user-flow-impacting TODO item against the actual executed integration/device or navigation/browser evidence. If no test covers that item directly, either add the missing test before delivery or record an approved waiver/deviation explaining why the item has no visible/runtime/user-flow behavior.
   - Cross-check non-visual refactors against the `Flow Evidence Planning Matrix`. If changed fields, DTOs, payloads, projections, validations, queries, settings, or capabilities feed a user flow, require direct runtime evidence for that flow before delivery.
   - Cross-check every browser/web-visible TODO item against the actual executed Playwright evidence when a Playwright suite exists. Missing or stale Playwright evidence blocks delivery; run the project-defined build/publish command from `foundation_documentation` or dependency-readiness notes, confirm the real browser-facing domain serves the refreshed bundle, then run the Playwright suite against that domain.
...
29. **Rule-Spirit Anti-Pattern Hunt (mandatory before delivery claim)**
   ...
   - Use deterministic scanner support when it fits the touched surface, starting with `bash delphi-ai/tools/rule_spirit_anti_pattern_scan.sh --repo <repo-root> --stack <stack>` and any stack-specific helpers it references. Treat scanner findings as review inputs; unresolved `P1|P2` judgment still belongs in this gate.
