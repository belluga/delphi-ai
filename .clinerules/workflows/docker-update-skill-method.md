...
<!-- Generated from `workflows/docker/update-skill-method.md` by `tools/sync_clinerules_mirrors.py`. Do not edit directly. -->

# Workflow: Update Skill Across Agent Surfaces

## Purpose
Update or create skills with one canonical process that keeps Cline, Claude, Codex, and Antigravity aligned.
...
5. If the skill is already exposed to Claude, or the change requires a new Claude mirror, sync it with `bash delphi-ai/tools/sync_claude_skill_mirrors.sh <skill-name>`.
6. If the skill is tracked in the public Codex mirror set (`bash delphi-ai/tools/list_public_codex_skill_mirrors.sh`), or the change requires adding it there, sync it with `bash delphi-ai/tools/sync_codex_public_skill_mirrors.sh <skill-name>`.
7. If the skill was created outside canonical surfaces (for example under `~/.codex/skills/**`), migrate it into `delphi-ai/skills/<skill-name>/SKILL.md`, sync any required Cline/Claude/public mirror, and remove/deprecate the out-of-surface copy to avoid drift.
8. If the skill introduces or changes operational behavior, update compatible rule/workflow files too:
...
9. If changed behavior defines/changes an API contract pattern (for example PATCH semantics), consolidate all contract surfaces:
...
10. If a new canonical pattern impacts existing code, add a mandatory side-job in the active TODO to align non-conforming areas (or record explicit exceptions with rationale/owner/next action).
11. If the changed skill is a workflow skill (`wf-*`), ensure a canonical workflow file exists under `delphi-ai/workflows/**` and a Cline workflow counterpart exists under `delphi-ai/.clinerules/workflows/**`.
12. Validate sync and compatibility:
   ...
   - Optional explicit diffs:
     - `diff -u delphi-ai/skills/<skill-name>/SKILL.md delphi-ai/.cline/skills/<skill-name>/SKILL.md`
     - `diff -u delphi-ai/skills/<skill-name>/SKILL.md delphi-ai/.claude/skills/<skill-name>/SKILL.md`
     - `diff -u delphi-ai/skills/<skill-name>/SKILL.md ~/.codex/skills/public/<skill-name>/SKILL.md` when the skill is listed by `list_public_codex_skill_mirrors.sh`
13. Report changed files and explicitly confirm consolidation for `Cline | Claude | Codex | Antigravity`.

## Outputs
- Updated skill in canonical and Cline/Claude/public-compatible locations as applicable.
...
- No mismatch between `delphi-ai/skills/<skill-name>/SKILL.md` and `delphi-ai/.claude/skills/<skill-name>/SKILL.md` when the skill is exposed to Claude.
- If the skill is tracked in the public Codex mirror list, no mismatch between `delphi-ai/skills/<skill-name>/SKILL.md` and `~/.codex/skills/public/<skill-name>/SKILL.md`.
