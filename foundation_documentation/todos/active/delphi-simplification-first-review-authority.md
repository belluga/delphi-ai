...
| `PR-16-architecture-review-status-enum` | closeout review | medium | release-blocker | fix in current TODO | fixed | Required reviews accept only canonical `no_material_findings`/`findings_integrated`; each waived review requires its dedicated human approval/reference field, and generic approval text or allow-waivers cannot bypass it. |
| `PR-17-p1-negative-phrase` | closeout review | medium | release-blocker | fix in current TODO | fixed | One authority-owned parser evaluates only canonical review Findings/Resolution clauses: each remaining P1/P2 clause must be clean/resolved/fixed/integrated with no dirty marker; ambiguous or dirty clauses block. |
