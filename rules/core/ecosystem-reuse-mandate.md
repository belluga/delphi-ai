...
1. **Proprietary Package Scan:** Before implementing any feature, the agent must query proprietary packages through `bash delphi-ai/tools/query_packages.sh --project-root <path> ...` and read relevant package READMEs via the CLI detail mode. Direct agent reads of registry YAML files are not authorized. See companion rule `paced.core.package-first`.
