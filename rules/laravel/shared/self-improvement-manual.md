...
## Notes
During self-improvement closure, post-session review still applies; the special case is that downstream mandate edits are deferred until the instruction-only session is over. Any temporary self-improvement work ledger under `delphi-ai/artifacts/tmp/` remains non-authoritative and must be deleted or intentionally refreshed at closure.
