...
| `github-stage-promotion-orchestrator` | `already-backed` | Existing support via [`github_stage_promotion_preflight.sh`](../tools/github_stage_promotion_preflight.sh), [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh), and [`github_promotion_completion_guard.sh`](../tools/github_promotion_completion_guard.sh); run the preflight first so wrong-base source branches or invalid submodule-only lane shapes trigger a TEACH runtime blocker before any PR is opened, use the snapshot for live PR/check evidence, and require the completion guard before claiming the lane is finished so Flutter/Laravel promotions cannot be closed while Docker finalization is still missing. |
