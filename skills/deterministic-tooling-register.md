...
| `wf-docker-todo-driven-execution-method` | `partial-tool` | Tactical execution governance, including keeping the same governing TODO through `promotion_lane/`, choosing the appropriate promotion skill, deciding when a producer triggers the `Frontend / Consumer Matrix`, and judging review findings remains human-led; delivery closure is supported by [`todo_completion_guard.py`](../tools/todo_completion_guard.py), while [`rule_spirit_anti_pattern_scan.sh`](../tools/rule_spirit_anti_pattern_scan.sh) supports the Rule-Spirit Anti-Pattern Hunt. CI-equivalent scoping, waiver validity, and architectural P1/P2 judgment remain human-led. |
