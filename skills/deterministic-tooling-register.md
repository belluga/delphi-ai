...
| `wf-docker-todo-driven-execution-method` | `partial-tool` | Tactical execution governance, including keeping the same governing TODO through `promotion_lane/`, choosing the appropriate promotion skill, and deciding when a backend/settings/payload/projection/capability producer triggers the `Frontend / Consumer Matrix`, remains judgment-led; delivery closure is supported by [`todo_completion_guard.py`](../tools/todo_completion_guard.py), which enforces criterion-specific completion evidence for every DoD and Validation Steps item, requires visible/interactive criteria to name item-specific integration/device or navigation/browser evidence, requires browser/web-visible criteria to name source-owned Playwright spec + runner evidence when a Playwright suite exists, requires visible CRUD/mutation criteria to name local non-main mutation evidence before delivery claims are trusted, and now blocks TODO delivery/promotable claims when the local CI-equivalent suite matrix is missing or unpassed. A future deterministic helper could check deeper suite-to-workflow semantic equivalence, but consumer necessity, waiver validity, and CI-surface scoping remain judgment-led. |
