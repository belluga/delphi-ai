...
| `wf-docker-subagent-worktree-reconciliation-method` | `partial-tool` | Branch/worktree scaffolding could be scripted later, but ownership partitioning, checkpoint acceptance, merge strategy, preserving the same governing TODO through reconciliation/promotion follow-through, and consolidated validation remain orchestrator judgment. |
| `wf-docker-todo-driven-execution-method` | `skill-only` | Tactical execution governance, including keeping the same governing TODO through `promotion_lane/` and choosing the appropriate promotion skill, can be supported by checks but not replaced by tooling. |
