...
| `rule-docker-shared-todo-driven-execution-model-decision` | `partial-tool` | Tactical authority remains judgment-led, but close-claim evidence is now supported by [`todo_completion_guard.py`](../tools/todo_completion_guard.py), which blocks delivery claims when DoD/Validation criteria lack row-level evidence, evidence is placeholder/aggregate-only, literal artifacts such as UI controls/routes/endpoints/schemas/runtime targets are not proven, visible/interactive criteria lack item-specific integration/device or navigation/browser evidence, browser/web-visible criteria lack source-owned Playwright spec + runner evidence when a Playwright suite exists, visible CRUD/mutation criteria lack local non-main mutation evidence, the Local CI-Equivalent Suite Matrix is missing/unpassed, or the Pipeline/Copilot P1/P2 Preflight / Rule-Spirit Anti-Pattern Hunt sections are missing, unexecuted, or record unresolved P1/P2 findings. |
...
| `github-stage-promotion-orchestrator` | `already-backed` | Umbrella router for manual `dev-only|through-stage` promotion. Phase details now live in `github-stage-promotion-*` phase skills for progressive disclosure. Existing support via [`github_promotion_contract_init.sh`](../tools/github_promotion_contract_init.sh), [`github_stage_promotion_scenario_classifier.py`](../tools/github_stage_promotion_scenario_classifier.py), [`github_promotion_diff_guard.sh`](../tools/github_promotion_diff_guard.sh), [`github_promotion_action_guard.sh`](../tools/github_promotion_action_guard.sh), guarded mutation wrappers, [`github_stage_promotion_preflight.sh`](../tools/github_stage_promotion_preflight.sh), [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh), and [`github_promotion_completion_guard.sh`](../tools/github_promotion_completion_guard.sh); the guards now block `bot/next-version -> stage` and generated `web-app` PR mutation. For `dev-only`, stop at healthy requested source repo(s) on `dev` unless Docker finalization was explicitly included; for `through-stage`, require Docker finalization and the stage completion guard before claiming completion. |
...
| `github-stage-promotion-contract-preflight` | `already-backed` | Phase skill supported by contract init, source preflight, action/diff guards, and guarded wrappers. The wrappers enforce action/diff policy but do not prove PR check or review readiness. |
...
| `github-stage-promotion-docker-finalization` | `already-backed` | Phase skill for app through-stage Docker gitlink follow-through; supported by bot recovery, stage promotion preflight, and `github_promotion_completion_guard.sh`. |
| `github-stage-promotion-failure-review` | `skill-only` | Failure triage and root-cause classification remain judgment-led; snapshots, local CI-equivalent commands, Copilot evidence, and independent critique can support but not replace the finding decision. |
| `github-stage-promotion-closeout-report` | `already-backed` | Phase skill for dev-only or through-stage completion reporting, TODO closeout status, and completion guard evidence when stage is in scope. |
| `github-main-promotion-orchestrator` | `already-backed` | Existing support via [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh) plus [`github_promotion_completion_guard.sh`](../tools/github_promotion_completion_guard.sh); use the snapshot to collect the current branch/PR/check evidence before manual `stage -> main` decisions, then require the completion guard before claiming completion so Docker finalization and Flutter web follow-through are validated deterministically instead of assumed. |
...
| `wf-docker-independent-critique-method` | `skill-only` | Independent no-context critique remains reviewer-driven; [`audit_escalation_guard.py`](../tools/audit_escalation_guard.py) now decides when the lane is required and how aggressive the package depth must be. |
...
| `wf-docker-todo-approval-gates-method` | `already-backed` | Phase skill for plan review and approval gates, supported by [`audit_escalation_guard.py`](../tools/audit_escalation_guard.py), [`subagent_review_dispatch.py`](../tools/subagent_review_dispatch.py), [`subagent_review_merge.py`](../tools/subagent_review_merge.py), and `audit-protocol-triple-review` when the floor requires it. |
| `wf-docker-todo-execution-boundary-method` | `skill-only` | Phase skill for approved-boundary execution and scope-change control. Tooling can support profile/path checks, but boundary judgment remains human-led. |
| `wf-docker-todo-delivery-gates-method` | `partial-tool` | Phase skill for delivery evidence, CI-equivalent execution, P1/P2 preflight, Rule-Spirit hunt, audits, and final guard. Supported by [`todo_completion_guard.py`](../tools/todo_completion_guard.py), [`rule_spirit_anti_pattern_scan.sh`](../tools/rule_spirit_anti_pattern_scan.sh), and audit/test/debt helpers. |
| `wf-docker-todo-closeout-promotion-method` | `skill-only` | Phase skill for blocked-state, canonicalization, promotion-lane, and closeout decisions. Promotion-specific deterministic support remains in the GitHub promotion workflows. |
