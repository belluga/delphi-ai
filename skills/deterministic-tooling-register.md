...
| `audit-protocol-triple-review` | `already-backed` | Existing support via [`triple_audit_session.py`](audit-protocol-triple-review/scripts/triple_audit_session.py) plus [`subagent_review_dispatch.py`](../tools/subagent_review_dispatch.py) and [`subagent_review_merge.py`](../tools/subagent_review_merge.py); use the session runner to create/resume round state, validate result files, and classify `clean|needs_resolution|needs_adjudication` while keeping adjudication judgment in the skill. |
...
| `github-stage-promotion-orchestrator` | `already-backed` | Existing support via [`github_promotion_contract_init.sh`](../tools/github_promotion_contract_init.sh), [`github_promotion_diff_guard.sh`](../tools/github_promotion_diff_guard.sh), [`github_promotion_action_guard.sh`](../tools/github_promotion_action_guard.sh), the guarded mutation wrappers, [`github_stage_promotion_preflight.sh`](../tools/github_stage_promotion_preflight.sh), [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh), and [`github_promotion_completion_guard.sh`](../tools/github_promotion_completion_guard.sh); create a local contract first so the authorized scope (`dev-only|through-stage`) and approvals are explicit, force local/manual commit-push-PR actions through the guarded wrappers, block gitlinks plus unauthorized CI/promotion-tooling drift with TEACH output, then run the preflight first so wrong-base source branches or invalid submodule-only lane shapes stop before any PR is opened, use the snapshot for live PR/check evidence, and require the completion guard before claiming the lane is finished so Flutter/Laravel promotions cannot be closed while Docker finalization is still missing. |
...
| `wf-docker-subagent-worktree-reconciliation-method` | `partial-tool` | Existing support via [`orchestration_plan_completion_guard.py`](../tools/orchestration_plan_completion_guard.py), which blocks incomplete orchestration plans before approval/execution claims, including missing acceptance traceability, untracked DoD/validation criteria, unapproved spec-marker substitutions, and orchestrator-owned implementation rows; and [`orchestration_delivery_guard.py`](../tools/orchestration_delivery_guard.py), which blocks delivery claims when traceability evidence is incomplete, runtime/browser/device freshness is unproven, named artifacts lack matching evidence, validation rows are missing, or the orchestrator is listed as implementation owner. Branch/worktree scaffolding and checkpoint-manifest validation could be scripted later, but checkpoint acceptance, checkpoint lifecycle status, merge strategy, preserving the same governing TODO through reconciliation/promotion follow-through, preventing infinite branch accumulation, and interpreting consolidated validation remain orchestrator judgment. |
| `wf-docker-todo-driven-execution-method` | `partial-tool` | Tactical execution governance, including keeping the same governing TODO through `promotion_lane/` and choosing the appropriate promotion skill, remains judgment-led; delivery closure is supported by [`todo_completion_guard.py`](../tools/todo_completion_guard.py), which enforces criterion-specific completion evidence for every DoD and Validation Steps item, requires visible/interactive criteria to name item-specific integration/device or navigation/browser evidence, requires browser/web-visible criteria to name source-owned Playwright spec + runner evidence when a Playwright suite exists, and requires visible CRUD/mutation criteria to name local non-main mutation evidence before delivery claims are trusted. |
