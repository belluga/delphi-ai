...
| `github-stage-promotion-orchestrator` | `already-backed` | Existing support via [`github_promotion_contract_init.sh`](../tools/github_promotion_contract_init.sh), [`github_promotion_diff_guard.sh`](../tools/github_promotion_diff_guard.sh), [`github_promotion_action_guard.sh`](../tools/github_promotion_action_guard.sh), the guarded mutation wrappers, [`github_stage_promotion_preflight.sh`](../tools/github_stage_promotion_preflight.sh), [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh), and [`github_promotion_completion_guard.sh`](../tools/github_promotion_completion_guard.sh); create a local contract first so the authorized scope (`dev-only|through-stage`) and approvals are explicit, use the conservative default for `dev-only`, use `--gitlink-policy pipeline-only` for `through-stage` so the guard blocks feature/fix gitlinks while permitting only `bot/next-version -> dev` and subsequent `dev -> stage` lane propagation, force local/manual commit-push-PR actions through the guarded wrappers, block unauthorized CI/promotion-tooling drift with TEACH output, then run the preflight first so wrong-base source branches or invalid submodule-only lane shapes stop before any PR is opened, remove/reset stale `bot/next-version` only with operator approval when the branch already has the desired pins but is based on old `origin/dev`, rely on the dispatcher to recreate those pins instead of hand-editing gitlinks, treat `web-app` as derived artifact evidence only and never as a manually promoted/mutated repo, use the snapshot for live PR/check evidence, and require the completion guard before claiming the lane is finished so Flutter/Laravel promotions cannot be closed while Docker finalization is still missing. |
