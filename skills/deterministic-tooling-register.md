...
| `github-stage-promotion-orchestrator` | `already-backed` | Existing support via [`github_stage_promotion_preflight.sh`](../tools/github_stage_promotion_preflight.sh) plus [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh); run the preflight first so wrong-base source branches or invalid submodule-only lane shapes trigger a TEACH runtime blocker: exit `2` stops the lane, `context` exposes the branch evidence, and `resolution_prompt` tells the operator exactly how to repair the source before any PR is opened. Then use the snapshot to collect the live branch/PR/check evidence for manual promotion decisions. |
| `github-main-promotion-orchestrator` | `already-backed` | Existing support via [`github_stage_promotion_snapshot.sh`](../tools/github_stage_promotion_snapshot.sh); use it to collect the current branch/PR/check snapshot before manual `stage -> main` decisions, require a finding-scrutiny gate plus no-context critique for ambiguous blockers, and replay fixes back through the authoritative source branch before resuming `main`. |
