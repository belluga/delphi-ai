...
| `test-orchestration-suite` | `already-backed` | Existing support via [`test_orchestration_status_report.sh`](../tools/test_orchestration_status_report.sh); use it to enforce explicit required-stage accounting while keeping orchestration judgment human. Browser stale-bundle recovery stays skill-driven for now because the project-owned publish script and metadata shape vary by downstream repo, but the status report should still record build/publish and reprobe stages explicitly when browser evidence is required. The skill now also requires a frozen local CI-equivalent suite matrix so TODO/promotion claims cannot rely on targeted reruns instead of the same suites/jobs CI will execute. |
...
| `wf-docker-todo-driven-execution-method` | `partial-tool` | Tactical execution governance, including keeping the same governing TODO through `promotion_lane/`, choosing the appropriate promotion skill, deciding when a backend/settings/payload/projection/capability producer triggers the `Frontend / Consumer Matrix`, and judging the substance of review findings remains judgment-led; delivery closure is supported by [`todo_completion_guard.py`](../tools/todo_completion_guard.py), which enforces criterion-specific completion evidence for every DoD and Validation Steps item, requires visible/interactive criteria to name item-specific integration/device or navigation/browser evidence, requires browser/web-visible criteria to name source-owned Playwright spec + runner evidence when a Playwright suite exists, requires visible CRUD/mutation criteria to name local non-main mutation evidence, blocks delivery/promotable claims when the local CI-equivalent suite matrix is missing or unpassed, and now requires Pipeline/Copilot P1/P2 Preflight plus Rule-Spirit Anti-Pattern Hunt evidence with no unresolved P1/P2 findings. A future deterministic helper could check deeper suite-to-workflow semantic equivalence and static anti-pattern classes, but consumer necessity, waiver validity, CI-surface scoping, and architectural finding judgment remain human-led. |
...
| `wf-docker-update-skill-method` | `already-backed` | Existing support via [`list_public_codex_skill_mirrors.sh`](../tools/list_public_codex_skill_mirrors.sh), [`sync_cline_skill_mirrors.sh`](../tools/sync_cline_skill_mirrors.sh), [`sync_codex_public_skill_mirrors.sh`](../tools/sync_codex_public_skill_mirrors.sh), [`sync_clinerules_mirrors.sh`](../tools/sync_clinerules_mirrors.sh), and [`self_check.sh`](../tools/self_check.sh); use them to keep canonical, `.cline`, `.clinerules`, and tracked public Codex mirrors converged. |
