...
- Before the first promotion PR in a lane, run `copilot-pr-review` once the authoritative source branch is preflight-green. The mandatory order is: internal no-context subagents first, then bounded external cross-model confirmation when available, with Claude preferred when available. Treat all review output as evidence, not patch authority.
