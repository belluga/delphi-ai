...
5. **Closure**
   - Only after steps 1–4 are complete, acknowledge closure.
...
- Self Improvement Sessions must not modify `foundation_documentation/` during this review; any confirmed mandate or session-memory changes are deferred until the instruction-only session is closed.
