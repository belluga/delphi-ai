...
description: "Workflow: MUST use whenever the scope matches this purpose: Execute work via a tactical TODO in `foundation_documentation/todos/active/`, including mandatory planning and adherence gates before implementation and delivery."
---
...
22. **Performance & Concurrency Risk Assessment (mandatory before delivery)**
   - Load `wf-docker-performance-concurrency-validation-method` and apply the canonical `pcv-1` package.
   ...
   - Classify each lane independently as `required|recommended|not_needed`; do not use one shared validation decision.
   - For each lane, record the required row fields from `pcv-1`, including `trigger_reason_code`, `gate_deadline`, `min_evidence_rule_id`, `state`, `residual_risk`, `recorded_at_utc`, and `executor_id`.
   - `recommended` lanes still must resolve by gate. Only `trigger_result = not_needed` may use `state = not_applicable`.
   - If a lane enters `running|passed`, require a machine-checkable JSON artifact plus recorded `SHA-256`.
   - Prose-only evidence is invalid. Required-lane waivers require distinct `executor_id`, `approver_id`, and `reviewer_id`.
