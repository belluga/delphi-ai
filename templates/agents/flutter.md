...
* If the user asks what is in Flutter `Problems`, which diagnostics remain, or whether a screenshot of `Problems` reflects the current workspace state, the agent must query the bridge/extension directly and treat that live snapshot as the only local source of truth.
