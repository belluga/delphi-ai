...
| `tools/effort_selection_advisor.py` | python | Recommend an advisory effort tier and GOAL policy from a declared Delphi surface plus ambiguity/goals-supported flags without blocking execution. |
...
| `tools/github_promotion_action_guard.sh` | shell | Emit a TEACH runtime blocker for out-of-scope local promotion actions, direct lane-branch mutation, promotion attempts from orchestration-only `reconcile/*` branches, forbidden `bot/next-version` commit/push actions, forbidden `bot/next-version -> stage` PR movement, generated `web-app` PR mutation, and forbidden docs promotion based on a local promotion contract. |
| `tools/github_release_package_rollup_guard.py` | python | Validate that a release-package TODO still matches the live version membership under `active/<version>` plus `promotion_lane/<version>`, require every live child owner to be promotion-eligible, derive current per-repo live states against `origin/dev`, and emit a TEACH runtime recommendation for the opening promotion track before repo-specific source preflight starts. |
| `tools/github_promotion_contract_init.sh` | shell | Create a local promotion contract that makes lane scope and authorization explicit before guarded manual promotion actions, including separate flags for CI control-plane changes versus narrow CI test-harness workflow adjustments. |
