...
| `tools/github_promotion_action_guard.sh` | shell | Emit a TEACH runtime blocker for out-of-scope local promotion actions, direct lane-branch mutation, forbidden `bot/next-version` commit/push actions while allowing authorized PR movement, and forbidden docs promotion based on a local promotion contract. |
