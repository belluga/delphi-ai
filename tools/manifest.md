...
| `tools/github_promotion_diff_guard.sh` | shell | Emit a TEACH runtime blocker for gitlinks outside the allowed `bot/next-version -> dev` or `dev -> stage` pipeline path, unauthorized CI changes, and unauthorized promotion-tooling changes found in a staged/worktree/range diff against a local promotion contract. |
