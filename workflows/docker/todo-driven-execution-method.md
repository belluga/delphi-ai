...
4. **Align TODO to the current canonical status schema**
...
6. **Confirm canonical module anchors (mandatory)**
...
7. **Classify complexity and checkpoint policy**
...
8. **Run module-first TODO scan and raise pending decisions (refinement)**
...
9. **Freeze Decision Baseline (mandatory)**
...
10. **Run Module Coherence Gate (mandatory before approval)**
...
11. **Resolve COMMENT blocks (mandatory gate)**
...
12. **Build Assumptions Preview (mandatory before plan review)**
...
13. **Build Execution Plan (mandatory before `APROVADO`)**
...
14. **Run Plan Review Gate (mandatory for `medium|big`; abbreviated for low-risk `small`)**
...
16. **Rules Acknowledgement / Ingestion (mandatory after `APROVADO` and before execution)**
...
18. **Decision Adherence Gate (mandatory before delivery)**
...
19. **Security Risk Assessment (mandatory before delivery)**
...
20. **Delivery Confidence Gate (mandatory for `✅ Production-Ready`)**
...
22. **Verification Debt Audit (required before close for `medium|big` or when debt signals exist)**
...
23. **Blocked-state update (mandatory when pausing blocked)**
...
24. **Module Consolidation Gate (mandatory before close)**
